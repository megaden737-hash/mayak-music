      ...
      <div className="grid gap-4 md:grid-cols-2">
        {DIRECTIONS.map((d, i) => (
          <Link
            key={d.slug}
            href={d.href}
            className="group relative overflow-hidden rounded-3xl border border-white/10 bg-navy-elevated"
          >
            ...
            <div className="relative flex min-h-[280px] flex-col justify-end p-6 md:min-h-[320px] md:p-8">
              <div className="mb-auto flex items-start justify-between">
                ...
                <span className="rounded-full border border-white/10 bg-black/30 px-3 py-1 text-xs text-gold backdrop-blur">
                  {d.price}
                </span>
              </div>

              <div className="text-xs tracking-[0.16em] text-white/45 uppercase">
                {d.subtitle}
              </div>
